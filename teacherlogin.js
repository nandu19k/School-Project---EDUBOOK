const teacherCollection = require('./db').db().collection("teacherss")
...
Teacher.prototype.validate = function() {
  ...
  if(this.data.password != this.data.psw){this.errors.push("Your password is not matching")}
    ...
    if ( this.data.username.length ==10 && validator.isNumeric(this.data.username)) {
      let usernameExists = await teacherCollection.findOne({username: this.data.username})
...
Teacher.prototype.login = function() {
  return new Promise((resolve, reject) => {
    this.cleanUp()
    teacherCollection.findOne({username: this.data.username}).then((attemptedUser) => {
